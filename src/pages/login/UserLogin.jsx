...
const UserLogin = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const result = await loginUser(email, password, rememberMe, deviceToken);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-600 to-indigo-700 px-4">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
        ...
        <div className="text-sm text-center mt-4 space-x-4">
          <a href="/register?role=user" className="text-indigo-600 hover:underline">
            Kayıt Ol
          </a>
          <a href="/forgot-password?role=user" className="text-indigo-600 hover:underline">
            Şifremi Unuttum
          </a>
        </div>
